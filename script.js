...
function actualizarTablaProyeccion(costosFijos, totalPuntoDeEquilibrio, totalCostoVariablePonderado, totalPrecioVentaPonderado, totalDemandaMensual) {
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    if (i === 1) {
      cantidad = totalPuntoDeEquilibrio;
    } else {
      cantidad += 10;
    }
...
function mostrarExplicacion(totalPuntoDeEquilibrio, totalDemandaMensual) {
  ...
  if (totalDemandaMensual < totalPuntoDeEquilibrio) {
    ...
    Puedes implementar estrategias de marketing, mejorar la eficiencia en la producción/ventas o buscar opciones para disminuir costos fijos y variables.`;
  ...
  alertaExplicacion.innerHTML = `<p>${mensaje1}</p><p>${mensaje2}</p><p>${mensaje3}</p>`;
